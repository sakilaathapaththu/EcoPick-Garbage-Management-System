...
export default function TrackingFillingdetails() {
  const [details, setDetails] = useState(null);
  const [expanded, setExpanded] = useState(false);
  const [filledCapacity, setFilledCapacity] = useState(0); // Declare filledCapacity state
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const totalCapacity = response.data?.totalCapacity || 0;
        const emptyCapacity = response.data?.emptyCapacity || 0;
        const filledCapacity = totalCapacity - emptyCapacity;

        setFilledCapacity(filledCapacity); // Set filledCapacity state
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                <Typography
                  ...
                  sx={{ paddingBottom: 2 }}
                >
                  Filled Capacity: {filledCapacity} {/* Use filledCapacity state */}
                </Typography>
              </Grid>
            </Grid>
